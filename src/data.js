export const personalInfo = {
  name: "Tushar Shinde",
  phone: "+91-9021644331",
  email: "tusharshinderajput2004@gmail.com",
  role: "Aspiring Software Developer",
  summary: "Aspiring Software Developer with strong foundation in backend development using Java, Spring Boot, and relational databases. Experienced in building REST APIs and full-stack applications using React and Docker. Passionate about solving real-world problems through scalable backend systems and efficient code.",
  github: "https://github.com",
  linkedin: "https://linkedin.com"
};

export const skillsData = {
  programming: ["Java", "C", "JavaScript"],
  frontend: ["HTML", "CSS", "JavaScript", "React"],
  backend: ["Spring Boot", "JSP", "Servlets", "JDBC"],
  databases: ["MySQL", "Oracle", "MongoDB"],
  tools: ["Git", "GitHub", "Docker", "Postman"],
  core: ["Data Structures & Algorithms", "OOP", "REST APIs", "Database Design"]
};

export const projectsData = [
  {
    title: "E-Commerce Web Application",
    techStack: ["React", "Spring Boot", "MySQL", "Material UI", "Tailwind CSS"],
    description: [
      "Full-stack e-commerce platform",
      "User authentication system",
      "Product catalog and cart management",
      "Order processing system",
      "REST API integration",
      "MySQL database integration"
    ],
    github: "#",
    live: "#"
  },
  {
    title: "Social Media Web Application",
    techStack: ["React", "Clerk Auth", "ImageKit", "Inngest"],
    description: [
      "User profiles and posts",
      "Follow/unfollow system",
      "Friend requests and feeds",
      "Image optimization using ImageKit",
      "Background jobs using Inngest",
      "User search and discovery"
    ],
    github: "#",
    live: "#"
  }
];

export const experienceData = [
  {
    role: "Technical Trainer",
    company: "Seed Infotech",
    duration: "Jun 2025 – Present",
    details: [
      "Teaching C, HTML, CSS, JavaScript, DSA",
      "Helping students with problem-solving"
    ]
  },
  {
    role: "Full Stack Intern",
    company: "YHills",
    duration: "Mar 2025 – Jun 2025",
    details: [
      "Built full stack applications",
      "Worked on frontend + backend integration"
    ]
  },
  {
    role: "Software Development Intern",
    company: "SETTribe LLP",
    duration: "Dec 2024 – Jun 2025",
    details: [
      "Java + React training",
      "Learned SDLC and version control"
    ]
  }
];

export const educationData = [
  {
    degree: "B.Tech CSE",
    institution: "G. Wanjari College, Nagpur",
    duration: "2021–2025",
    score: "CGPA: 7.73"
  },
  {
    degree: "12th Grade",
    institution: "State Board",
    duration: "Completed",
    score: "80.3%"
  },
  {
    degree: "10th Grade",
    institution: "State Board",
    duration: "Completed",
    score: "70.43%"
  }
];

export const certificationsData = [
  "AWS Cloud Quest: Cloud Practitioner",
  "Oracle Cloud Infrastructure AI Foundations Associate (2025)",
  "Java Internship – iStudio",
  "Java Full Stack Internship – EduSkills",
  "Full Stack Internship – YHills"
];
